...
cfg.p_lp              = 4;       % LP lag order: I=p for y own-lags;
                                 % controls pre-expanded to p lag columns
                                 % then passed with J=0 (no lagged shocks)
...
for i = 1:numel(outcome_ids)
    yid  = outcome_ids{i};
    ylab = outcome_labels{i};   % renamed from `ylabel` to avoid shadowing builtin
    ...
    results_world.(yid) = estimate_lp_via_ir_jorda( ...
        y, z_world, controls, sample, cfg, ylab, 'world');

    results_threat.(yid) = estimate_lp_via_ir_jorda( ...
        y, z_threat, controls, sample, cfg, ylab, 'threat');

    results_act.(yid) = estimate_lp_via_ir_jorda( ...
        y, z_act, controls, sample, cfg, ylab, 'act');
    ...
    tmp.outcome_label = repmat(string(ylab), height(tmp), 1);
...
function res = estimate_lp_via_ir_jorda(y, z, controls, sample, cfg, y_label, shock_tag)
    ...
    [beta, se, lb, ub, nobs] = deal(nan(H+1, 1));
