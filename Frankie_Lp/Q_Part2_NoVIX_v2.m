...
%
% Shared utilities used (all in ../../code):
%   lagmatrix.m, lrv_nw.m, norminv.m
%% =======================================================================


clear; clc; close all;

% Add MATLAB function codes
...
%% Load a MATLAB table out of a .mat file --------------------------------
...
%% (1) Best-effort extraction of a date / quarter label column -----------
%
% Inputs:
%   DB : MATLAB table.
...
%
% Sandwich: V = (X'X)^{-1} S (X'X)^{-1} where S accumulates the
% heteroskedastic outer products plus Bartlett-weighted lag terms.
%
% Vectorised via lrv_nw.m: lrv_nw(X.*u, bw) returns S/T (the same S the
% previous double-loop implementation accumulated, divided by T).
% Therefore:
%   SigmaX = X'X / n
%   Omega  = lrv_nw(X .* u, bw)            % = S/n
%   V      = (n/(n-k)) * SigmaX^{-1} Omega SigmaX^{-1} / n
% matches the original up to the (n/(n-k)) finite-sample correction.
%
% Inputs:
%   y  : n-by-1 dependent variable (no NaNs expected here).
%   X  : n-by-k regressor matrix including any constant.
%   bw : non-negative integer Bartlett truncation lag.
%
% Outputs:
%   b  : k-by-1 OLS coefficients.
%   se : k-by-1 HAC standard errors (zero-floored before sqrt).
