...
function res = estimate_lp_level_change_single_shock(y, z, gpr_level, controls, sample, cfg, y_label, shock_tag)
    ...
    for h = 0:H
        ...
        % HAC bandwidth: Lazarus, Lewis, Stock & Watson (2018, JBES)
        % fixed-b rule for the Bartlett kernel: S = round(0.18 * T),
        % i.e. b = S/T ~ 0.18. Wider than the consistent-bandwidth rule
        % S = ceil(1.3 * sqrt(T)); produces HAR-robust inference at the
        % cost of some power. Capped at n-1 for safety.
        n_lp = size(X,1);
