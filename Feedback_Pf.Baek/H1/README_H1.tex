...
\noindent Adding UNRATE as a business-cycle control leaves the IRF shapes largely unchanged. The IP responses remain negative, and the CPI responses remain near zero. This suggests that the baseline results are not driven by business-cycle confounding.
...
\noindent Further adding VIX (financial uncertainty) produces similar results. The IP contraction persists, and the CPI response remains insignificant. The shorter sample ($\approx$\,432 months due to VIX availability) does not qualitatively alter the findings.

\subsection{Specification Comparison}

Figure~\ref{fig:comparison} overlays the point estimates $\hat{\beta}_h$ from all three specifications. The close alignment of the three lines across all shock-outcome pairs confirms the \textbf{stability of the LP estimates to the inclusion of potential confounders}.
