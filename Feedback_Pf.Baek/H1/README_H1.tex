...
\begin{table}[H]
\centering
\caption{Code and Script Files}
\label{tab:code}
\small
\begin{tabularx}{\textwidth}{@{}l l X@{}}
\toprule
\textbf{File} & \textbf{Location} & \textbf{Role} \\
\midrule
\texttt{download\_data.sh}    & \texttt{scripts/} & Download raw data from FRED + C\&I website \\
\texttt{s01\_load\_data.m}    & \texttt{scripts/} & Load, aggregate daily$\to$monthly, merge, log transform, save \texttt{.mat} \\
\texttt{s02\_explore\_data.m} & \texttt{scripts/} & Summary stats, time series plots, ADF tests, correlations, ACF \\
\texttt{s03\_run\_h1.m}       & \texttt{scripts/} & \textbf{Main analysis}: lag selection + baseline LP + robustness 1\,\&\,2 + comparison plots \\
\addlinespace
\texttt{lp\_estimate.m}       & \texttt{code/} & LP estimation: horizon-by-horizon OLS + Newey--West SE \\
\texttt{lp\_newey\_west.m}    & \texttt{code/} & Newey--West HAC standard error function \\
\texttt{lp\_lag\_select.m}    & \texttt{code/} & VAR-based AIC/BIC lag selection \\
\texttt{lp\_plot\_irf.m}      & \texttt{code/} & IRF plotting with shaded confidence bands \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Step} & \textbf{File} & \textbf{What it does} & \textbf{Output} \\
\midrule
1 & \texttt{download\_data.sh} & Download raw CSV/XLS from FRED and C\&I website & \texttt{data/raw/*} \\
2 & \texttt{s01\_load\_data.m} & Load $\to$ daily-to-monthly $\to$ merge $\to$ log transform $\to$ save & \texttt{h1\_baseline.mat} \\
3 & \texttt{s02\_explore\_data.m} & Summary stats, time series plots, ADF tests, correlations, ACF & \texttt{output/fig\_*.png} \\
4 & \texttt{s03\_run\_h1.m} & Lag selection $\to$ Baseline LP $\to$ Robustness 1\,\&\,2 $\to$ comparison & \texttt{output/fig\_h1\_*.png} \\
\bottomrule
\end{tabularx}
\end{table}
