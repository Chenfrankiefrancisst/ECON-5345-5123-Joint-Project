...
├── data/
│   ├── raw/                 downloaded CSV/XLS (run download_data.sh)
│   ├── h1_baseline.mat      master monthly panel (created by s01)
│   └── README.md            data dictionary with variable details
├── scripts/
│   ├── download_data.sh     fetch raw data from FRED + Caldara website
│   ├── s01_load_data.m      load, merge, transform → h1_baseline.mat
│   └── s02_explore_data.m   summary stats, plots, ADF tests, correlations
├── code/                    reusable LP functions
│   ├── lp_estimate.m        horizon-by-horizon Local Projection (Jorda 2005)
│   ├── lp_newey_west.m      Newey-West HAC standard errors
│   └── lp_plot_irf.m        IRF plotting with confidence bands
└── output/                  figures, tables (generated by scripts)
```

> **H1 baseline details** (data, model spec, parameters, deliverables): see [`README_H1.md`](README_H1.md) | LaTeX version: [`README_H1.tex`](README_H1.tex)
