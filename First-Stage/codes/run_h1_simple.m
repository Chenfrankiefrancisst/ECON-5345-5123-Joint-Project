...
% Macro transformations (log-difference)
master.dlog_ip  = [NaN; 100 * diff(log(master.INDPRO))];
master.infl     = [NaN; 100 * diff(log(master.CPI))];
master.dlog_wti = [NaN; 100 * diff(log(master.WTI))];

% Levels
master.ffr_level    = master.FFR;                           % level
master.unrate_level = master.UNRATE;                        % level
master.dlog_vix = [NaN; 100 * diff(log(master.VIX))];       % log difference
...
for i = 1:3
    [ACF(:,i), L] = autocorr(DATA(:,i), 'NumLags', 36);
    [PACF(:,i), L] = parcorr(DATA(:,i), 'NumLags', 36);
end
...
%% 5-3. Robustness 2 (+ UNRATE + dlog(VIX))

for s = 1:3
    
    shock = master.(shock_vars{s});
    X = [shock, master.unrate_level, master.dlog_vix];   % shock + UNRATE + dlog(VIX)
...
sgtitle('ADL Robustness 2: + UNRATE + dlog(VIX) (1-s.d. shock)');
...
%% 6-3. Robustness 2 (+ UNRATE + dlog(VIX))
...
sgtitle('LP Robustness 2: + UNRATE + dlog(VIX) (1-s.d. shock)');
