clear; clc; close all;
%% ============================================================
% PART 3AB: LEVEL LOCAL PROJECTIONS WITH POISSON EVENT-ARRIVAL RESIDUALS
% MATCHED TO RAW DUMMY LP CONTROLS
%
% Teacher-suggested specification:
%
%   y_{t+h} - y_{t-1} = alpha_h + beta_h z_t + Gamma_h controls_t + e_{t+h}
%
% where:
%   - y is a level variable.
%   - z_t is a standardized GPR shock.
%   - beta_h is already a level-change response, so DO NOT cumulate beta_h.
%   - lagged GPR LEVELS are included as controls, not lagged z-values.
%
% This version:
%   1. Uses the levels database Q_Levels_Database.csv.
%   2. Lets you choose aggregate shocks or country-specific shocks.
%   3. Adds COVID and oil-event onset dummy controls.
%   4. Does NOT save figures or results by default.
%% ============================================================
...
cfg.output_dir = 'Part3AB_LevelLP_PoissonEventResiduals_MatchedDummyControls';
...
% z_t is the standardized residual from an AR(p) forecasting equation in GPR LEVELS
% augmented with business-cycle controls:
%   GPR_t = a + rho_1 GPR_{t-1} + ... + rho_p GPR_{t-p} + gamma' B_t + u_t.
% Baseline B_t includes unemployment and FFR / policy rate.
% Set cfg.gpr_shock_control_timing = 'lagged' if you prefer B_{t-1}.
cfg.p_gpr_shock = 4;
...
cfg.run_event_residual_impulse_dashboards = true;      % new Poisson-residual impulse LPs
cfg.run_gpr_plus_event_residual_lp = true;             % joint LP: GPR shock + Poisson event surprise
...
cfg.make_combined_fig_12_14_16 = true;
...
fprintf('GPR shock extraction: AR(%d) in GPR levels + business-cycle controls (%s).\n', ...
    cfg.p_gpr_shock, cfg.gpr_shock_control_timing);
...
% This aligns GPR shock construction with the Poisson-GPR test:
% AR(4) residuals augmented with unemployment and FFR / policy rate.
[gpr_shock_controls, gpr_shock_control_names] = build_controls(DB, cfg.gpr_shock_control_aliases, T);
fprintf('GPR shock-extraction controls loaded: %s\n', strjoin_or_none(gpr_shock_control_names));
for j = 1:numel(gpr_shock_control_names)
    fprintf('  GPR shock control %-18s finite = %3d / %3d\n', gpr_shock_control_names{j}, sum(isfinite(gpr_shock_controls(:,j))), T);
end
...
for s = 1:size(cfg.shock_specs,1)
    ...
    innov = extract_ar_level_innovation_with_controls(gpr, sample, cfg.p_gpr_shock, ...
        gpr_shock_controls, cfg.gpr_shock_control_timing, sid);
...
if isfield(cfg, 'run_event_residual_impulse_dashboards') && cfg.run_event_residual_impulse_dashboards && ~isempty(dummy_aligned)
    ...
    poisson_innov = extract_ar_level_innovation_with_controls(poisson_gpr_level, sample, cfg.p_gpr_shock, ...
        gpr_shock_controls, cfg.gpr_shock_control_timing, cfg.event_residual_poisson_gpr_label);
    if cfg.standardize_shock
...
if cfg.save_results
    ...
    save(fullfile(cfg.output_dir, ['workspace_level_lp_' mode_tag '.mat']), ...
        'cfg','Z','GPR_levels','innovation_info','level_lp_results','level_lp_struct','control_names','gpr_shock_control_names','dummy_aligned','dummy_controls','dummy_control_names','dummy_info','event_impulse_results','event_impulse_struct','event_residual_results','event_joint_results','event_residual_struct','event_residual_diagnostics','event_residual_gpr_tests');
    fprintf('\nSaved results in: %s\n', cfg.output_dir);
else
    fprintf('\nNo files saved because cfg.save_results=false and cfg.save_figures=false.\n');
    fprintf('Results are available in workspace: level_lp_results, event_residual_results, event_joint_results, event_residual_gpr_tests, level_lp_struct, Z, innovation_info, dummy_aligned.\n');
end
