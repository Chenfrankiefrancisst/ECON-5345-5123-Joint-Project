clear; clc; close all;
% Generated version: Poisson-arrival shock is the main event regressor;
% Poisson log-intensity includes one lag of World, Threat, and Act GPR levels.
%% ============================================================
% PART 3AB: LEVEL LOCAL PROJECTIONS WITH POISSON ARRIVAL SHOCKS
% MAIN BASELINE: Poisson event-arrival shocks + GPR SHOCK CONTROLS; RAW N_t CHECKS ARE ROBUSTNESS
%
...
%   3. Constructs Poisson event-arrival shocks as standardized Pearson residuals.
%   4. Uses the Poisson arrival shock as the main event regressor.
%   5. Includes one lag of World, Threat, and Act GPR levels in the Poisson intensity equation.
%   6. Keeps raw N_t event-count LPs / raw-N controls as robustness checks.
%   7. Does NOT save figures or results by default.
%% ============================================================
...
cfg.output_dir = 'Part3AB_PoissonArrivalShock_Main_Lag1WorldThreatAct_CompactLP';
...
% Compact RHS for the Poisson-arrival-shock event LP.
% This compact specification is used inside the main event-arrival-shock LP block.
% It leaves the ordinary GPR-shock dashboards unchanged.
%
% Compact event-count LP controls follow:
%
%   y_{t+h} - y_{t-1}
%     = alpha_h + theta_h z_t^{WorldGPR} + beta_h z_t^{N,k} + delta_h' D_{t,-k}
%       + sum_{j=1}^2 phi_{h,j} y_{t-j}
%       + sum_{j=1}^2 psi_{h,j} WorldGPR_{t-j}
%       + Gamma_h' W_{t-1}
%       + sum_{j=1}^2 rho_{h,j} N_{t-j}^k + e_{t+h}.
%
% Notes:
%   - D_{t,-k} contains COVID_2020 and the other current event-onset dummies,
%     after dropping the current event's own dummy.
%   - W_{t-1} contains the lag-1 macro controls in cfg.control_aliases.
%   - No contemporaneous or lagged oil-price block is added to this compact LP.
%     Oil prices are still used in the first-stage extraction of z_t^{WorldGPR}.
%   - Current Unemp/FFR are also not added separately, because the compact LP
%     uses W_{t-1} and the GPR-shock extraction already controls current Unemp/FFR.
cfg.use_compact_event_count_lp = true;
cfg.p_dynamic_lp_event_count = 2;       % y lags, GPR-level lags, and N-count lags
cfg.p_macro_lp_event_count = 1;         % macro-control lags: W_{t-1} only
cfg.include_current_business_controls_event_count = false;
cfg.include_oil_controls_event_count = false;
...
% uses a Poisson event-arrival residualization as the main event impulse.
%
% IMPORTANT UPDATE IN THIS VERSION:
%   GPR is INCLUDED in the Poisson MLE intensity equation, including the
%   current World GPR shock and one lag of World, Threat, and Act GPR levels.
%   This removes the component of event arrivals predictable from aggregate GPR components:
%
%       N_t^k | I_{t-1} ~ Poisson(lambda_t^k)
%       log(lambda_t^k) = alpha_k + gamma_k z_t^{WorldGPR}
%           + psi_{k,W} WorldGPR_{t-1} + psi_{k,T} ThreatGPR_{t-1}
%           + psi_{k,A} ActGPR_{t-1} + rho_k N_{t-1}^k + eta_k' W_{t-1}.
%
% The LP impulse is the standardized Pearson residual:
%
%       z_t^{N,k} = standardize((N_t^k - lambda_hat_t^k) / sqrt(lambda_hat_t^k)).
%
% This residual is interpreted as the unexpected event-arrival component after
% removing the part predictable from GPR, lagged event counts, and macro controls.
cfg.run_event_onset_impulse_dashboards = false;        % old raw 0/1 dummy impulse LPs
cfg.run_event_residual_impulse_dashboards = true;      % main: Poisson-residual event-arrival shock LPs
cfg.run_gpr_plus_event_residual_lp = true;             % main: GPR shock + Poisson event-arrival shock
cfg.run_gpr_plus_event_count_lp = false;               % robustness only: GPR shock + raw event count N_t^k
cfg.run_poisson_arrival_rawN_control_robustness = true; % robustness: add current raw N_t^k as an additional control
...
% GPR series used both in the Poisson arrival-intensity equation
% and in the second-stage LP controls. The Poisson log-intensity includes
% the current World GPR shock plus one lag of World, Threat, and Act GPR levels.
cfg.event_residual_poisson_gpr_aliases = {'LGPR'};
cfg.event_residual_poisson_gpr_label = 'World';

% Include GPR in the Poisson MLE intensity equation so that the
% constructed event-arrival shock removes the part of event arrivals
% predictable from GPR. This addresses the comment that lagged GPR should
% appear in log(lambda_t) if GPR predicts event arrivals.
%
% Baseline residualization model:
%   log(lambda_t^k) = alpha_k + gamma_k z_t^{WorldGPR}
%       + psi_{k,W} WorldGPR_{t-1} + psi_{k,T} ThreatGPR_{t-1}
%       + psi_{k,A} ActGPR_{t-1}
%       + rho_k N_{t-1}^k + eta_k' W_{t-1}.
...
cfg.poisson_lagged_gpr_component_aliases = { ...
    {'LGPR'}, ...     % World GPR level
    {'LGPRT'}, ...    % Global geopolitical threats level
    {'LGPRA'} ...     % Global geopolitical acts level
};
cfg.poisson_lagged_gpr_component_labels = {'WorldGPR','ThreatGPR','ActGPR'};

% Test whether the Poisson residual event-arrival shock is stronger
...
cfg.poisson_gpr_timing = 'current'; % include z_t^{WorldGPR} in log(lambda_t)
...
if isfield(cfg, 'run_event_residual_impulse_dashboards') && cfg.run_event_residual_impulse_dashboards && ~isempty(dummy_aligned)
    ...
    % GPR shock and GPR level used both for the Poisson intensity
    % equation and for the second-stage LP controls.
    ...
    % GPR level components used in the Poisson arrival-intensity equation.
    % Row t enters log(lambda_t^k) through one-period lags: t-1.
    ...
    for dd = 1:numel(cfg.event_impulse_vars)
        ...
        % After constructing the event-arrival surprise from the Poisson
        % baseline, test whether this residual arrival shock is
        ...
        % Compact LP lag controls: macro controls plus lagged raw N_t^k.
        % The macro controls use cfg.p_macro_lp_event_count lags, while
        % N_t^k uses cfg.p_dynamic_lp_event_count lags.
        ...
        if isfield(cfg, 'run_gpr_plus_event_residual_lp') && cfg.run_gpr_plus_event_residual_lp
            ...
            for i = 1:size(cfg.outcome_specs,1)
                ...
                cfg_joint.shock_size_z2 = 1;               % one s.d. event-arrival shock

                % Compact RHS: z_t^{WorldGPR}, z_t^{N,k}, D_{t,-k},
                % y_{t-1:t-2}, WorldGPR_{t-1:t-2}, W_{t-1}, and
                % N_{t-1:t-2}^k. This keeps the LP consistent with the
                % Poisson log-intensity, which also includes lag-1 World/Threat/Act GPR levels.
                if isfield(cfg, 'use_compact_event_count_lp') && cfg.use_compact_event_count_lp
                ...
                % Robustness check: keep the Poisson arrival shock as the
                % coefficient of interest but additionally control for the
                % contemporaneous raw event count N_t^k. Because z_t^{N,k}
                % is mechanically related to N_t^k, this is intentionally a
                % demanding check and may be collinear for very sparse events.
                ...
                joint_titles{i} = sprintf('%s, Poisson arrival shock controlling GPR', ylabel);
            end
    ...
    % and plots component 2, i.e. the Poisson residual event-arrival
...
%       + lag-1 World/Threat/Act GPR levels
%       + lagged event counts
%       + lagged outcomes
...
if isfield(cfg, 'run_gpr_plus_event_count_lp') && cfg.run_gpr_plus_event_count_lp && ~isempty(dummy_aligned)
    ...
    % Oil prices are used above only to extract the World GPR shock.
    % In the compact N_t LP below, no separate oil-price block is added,
    % because Brent/WTI/Gasoline comove strongly with the oil outcomes and
    % can absorb the variation of interest in rare-event regressions.
    % Keep lag-1 oil controls only for the optional older full FWL switch.
    ...
    for dd = 1:numel(cfg.event_impulse_vars)
        ...
        for i = 1:size(cfg.outcome_specs,1)
            ...
            % Current RHS controls for the compact N_t regression.
            % D_{t,-k} is always included. In the compact formula used here,
            % no contemporaneous oil-price block is added, because Brent/WTI/
            % gasoline comove strongly with the oil outcomes. Oil prices are
            % already used in extracting z_t^{WorldGPR}.
            current_fwl_controls = dummy_controls_dd;

            if isfield(cfg, 'include_current_business_controls_event_count') && ...
                    cfg.include_current_business_controls_event_count
                current_fwl_controls = [current_fwl_controls, gpr_shock_current_controls]; %#ok<AGROW>
            end

            % Optional older full FWL behavior: if compact mode is disabled,
            % add current business controls and lag-1 oil controls as before.
            if ~(isfield(cfg, 'use_compact_event_count_lp') && cfg.use_compact_event_count_lp)
                ...
                current_fwl_controls = [dummy_controls_dd, gpr_shock_current_controls, oil_lag1_as_current(:, oil_keep)]; %#ok<AGROW>
            end
            ...
            % Compact control-count reduction for the N_t FWL regression:
            % controls_with_lagN = [macro controls, N_event].
            % The macro controls use cfg.p_macro_lp_event_count lags, while the
            % last column, N_event, keeps cfg_joint.p_lp lags.
            if isfield(cfg, 'use_compact_event_count_lp') && cfg.use_compact_event_count_lp
                cfg_joint.p_lp = cfg.p_dynamic_lp_event_count;
            end
            cfg_joint.p_control_lag_two_impulses = cfg.p_macro_lp_event_count;
...
function out = estimate_poisson_event_residual(N, z_gpr, gpr_level, controls, sample, cfg, label)
    ...
    % t must have N_t and lagged predictors. For consistency with the
    % compact LP, the Poisson log-intensity can include both current
    % z_t^{WorldGPR} and lag-1 World/Threat/Act GPR levels.
    p_gpr_level = 0;
        ...
        if p_gpr_level > 0 && ~isempty(gpr_level)
            ...
            for jj = 1:p_gpr_level
                for gg = 1:size(gpr_level,2)
                    ...
                    pred_names{end+1} = sprintf('%s_lag%d', base_name, jj); %#ok<AGROW>
                end
            end
        end
    end
...
function outtab = estimate_event_residual_gpr_relation(event_id, z_event_resid, z_gpr_shock, gpr_level_std, sample, cfg)
    % Diagnostic: test whether the Poisson residual arrival shock is still related to GPR when
...
function [Y, X, good, dbg] = build_lp_level_change_design_two_impulses(y, z1, z2, gpr_level, controls, dummy_controls, sample, p, h, cfg)
    ...
    % Default behavior: every column in controls uses p lags.
    % Optional behavior for the FWL raw-event-count N_t regression:
    %   controls = [macro controls, N_event]
    %   - macro controls use cfg.p_control_lag_two_impulses lags,
    %   - the last cfg.n_full_lag_controls_two_impulses columns keep the full p lags.
    % With cfg.p_lp=4, cfg.p_control_lag_two_impulses=2, and
    % cfg.n_full_lag_controls_two_impulses=1, the six macro controls use two lags,
    % while N_{t-1}^k,...,N_{t-4}^k are still included.
    p_control = p;
    ...
    % Columns:
    %   const
    %   z1_t = World GPR shock
    %   z2_t = raw N_t^k or Poisson event-arrival shock
    %   current dummy/current controls
    %   y_{t-1},...,y_{t-p}
    %   GPR_{t-1},...,GPR_{t-p}
    %   short-lag controls, e.g. macro controls: t-1 to t-p_control
    %   full-lag controls, e.g. event count N_t^k: t-1 to t-p
    t_grid = (p_max+1):(T-h);
    n = numel(t_grid);
