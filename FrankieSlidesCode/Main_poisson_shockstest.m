...
%   Test 2: Ljung-Box white-noise test
...
cfg.output_dir   = 'Poisson_event_shock_tests_output';
...
%% Controls
[current_controls, current_names] = build_controls(DB, cfg.current_control_aliases, T);
[lag1_oil_controls, lag1_oil_names] = build_controls(DB, cfg.lag1_oil_aliases, T);
[macro_controls, macro_names] = build_controls(DB, cfg.macro_control_aliases, T);

fprintf('GPR current controls: %s\n', strjoin_or_none(current_names));
fprintf('GPR lag-1 oil ctrls : %s\n', strjoin_or_none(lag1_oil_names));
fprintf('Poisson macro ctrls : %s\n', strjoin_or_none(macro_names));

%% World GPR shock
world_gpr = get_required_series(DB, cfg.world_gpr_aliases, T);
[z_world, world_info] = extract_ar_level_innovation(world_gpr, sample, cfg.p_gpr_shock, ...
    current_controls, lag1_oil_controls, cfg.standardize_shock);
fprintf('World GPR innovation: nobs=%d, R2=%.3f, resid_sd=%.4f\n', ...
    world_info.nobs, world_info.rsq, world_info.resid_sd);

%% Event counts
dummy_aligned = build_aligned_dummy_table(cfg, quarter_labels, has_quarter, T);

%% Granger candidates
[granger_X, granger_names] = build_granger_candidates(DB, cfg.granger_exog_aliases, T, z_world, world_gpr);
fprintf('Granger candidates  : %s\n', strjoin_or_none(granger_names));
...
%% Plot combined figure with bottom timeline
if isfield(cfg, 'make_arrival_plots') && cfg.make_arrival_plots
...
if cfg.save_results
    ...
    save(fullfile(cfg.output_dir, 'workspace_poisson_event_shock_tests.mat'), ...
        'cfg','Z_event','N_event','z_world','world_gpr','world_info', ...
...
function plot_event_arrivals_and_shocks(N_event, Z_event, cfg, quarter_labels, has_quarter, T)
    ...
    if isfield(cfg, 'save_figures') && cfg.save_figures
...
function x = get_series_if_exists(DB, aliases, T)
    ...
    for i = 1:numel(aliases)
        target = char(aliases{i});
        idx = find(strcmpi(names, target), 1);

        if isempty(idx)
            idx = find(startsWith(names, target, 'IgnoreCase', true), 1);
        end
...
function x = get_required_series(DB, aliases, T)
    x = get_series_if_exists(DB, aliases, T);

    if isempty(x)
        if ischar(aliases) || isstring(aliases)
            msg = char(aliases);
        else
            msg = strjoin(cellstr(aliases), ', ');
        end
        error('Required variable not found. Tried: %s', msg);
    end
end

function [X, names_out] = build_controls(DB, alias_list, T)
    X = zeros(T,0);
    names_out = {};

    for i = 1:numel(alias_list)
        x = get_series_if_exists(DB, alias_list{i}, T);

        if isempty(x)
            warning('Control not found and skipped: %s', strjoin(alias_list{i}, ', '));
            continue;
        end

        X = [X, x]; %#ok<AGROW>
        names_out{end+1} = alias_list{i}{1}; %#ok<AGROW>
    end
end

function [X, names_out] = build_granger_candidates(DB, alias_list, T, z_world, world_gpr)
    X = zeros(T,0);
    names_out = {};

    for i = 1:numel(alias_list)
        primary = alias_list{i}{1};

        if strcmpi(primary, 'z_WorldGPR')
            x = z_world(:);
            name = 'z_WorldGPR';
        elseif strcmpi(primary, 'WorldGPR')
            x = world_gpr(:);
            name = 'WorldGPR';
        else
            x = get_series_if_exists(DB, alias_list{i}, T);
        ...
        if isempty(x)
            warning('Granger candidate skipped: %s', strjoin(alias_list{i}, ', '));
            continue;
        end

        X = [X, x(:)]; %#ok<AGROW>
        names_out{end+1} = name; %#ok<AGROW>
    end
end
...
function [z, info] = extract_ar_level_innovation(gpr, sample, p, current_controls, lag1_oil_controls, do_std)
    gpr = double(gpr(:));
    T = numel(gpr);

    if isempty(current_controls), current_controls = zeros(T,0); end
    if isempty(lag1_oil_controls), lag1_oil_controls = zeros(T,0); end

    t_grid = ((p+1):T)';
    X_ar = zeros(numel(t_grid), p);

    for j = 1:p
        X_ar(:,j) = gpr(t_grid-j);
    end

    X = [ones(numel(t_grid),1), X_ar, current_controls(t_grid,:), lag1_oil_controls(t_grid-1,:)];
    Y = gpr(t_grid);

    valid = sample(t_grid) & isfinite(Y) & all(isfinite(X),2);
    Xg = X(valid,:);
    Yg = Y(valid);
    t_used = t_grid(valid);

    if size(Xg,1) <= size(Xg,2)
        error('Not enough observations for World GPR AR(%d) extraction.', p);
    end
    ...
    if do_std
        z = (resid - mean(u,'omitnan')) ./ std(u,'omitnan');
    else
        z = resid;
    end
    ...
    info = struct();
    info.nobs = numel(Yg);
    info.rsq = 1 - sum(u.^2) / sum((Yg - mean(Yg)).^2);
    info.resid_sd = std(u);
end
